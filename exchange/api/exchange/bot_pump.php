...
if ($price_need < $token_price) {
    ...
    if ($price != null) {
        ...
        $key = dataWalletKey($domain, $address, $password, dataGet([$domain, wallet, $address, prev_key]));
        $next_hash = dataWalletHash($domain, $address, $password, $key);
    ...
    if ($price != null) {
        ...
        $amount_base = round($amount_base, 2);
        //echo ($price * $amount_base) . " $price $amount_base\n";
        $key = dataWalletKey(usdt, $address, $password, dataGet([usdt, wallet, $address, prev_key]));
        $next_hash = dataWalletHash(usdt, $address, $password, $key);
        $response = http_post("localhost/" . $domain . "/api/exchange/place.php", [
            is_sell => "0",
            address => $address,
            price => $price,
